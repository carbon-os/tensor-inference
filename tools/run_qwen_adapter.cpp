...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // ── 4. compose base + adapter ─────────────────────────

        models::qwen::QwenAdapterModel model(
            std::move(base),
            std::move(adapter_weights));
        ...
        const inference::GenerateOptions base_opts {
            ...
            .on_token       = [&](int32_t tok) -> bool {
                std::cout << tokenizer.decode({tok}, true) << std::flush;
                return true;
            },
        };
