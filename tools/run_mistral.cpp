// ─────────────────────────────────────────────────────────────
//  run_mistral — end-to-end Mistral/Ministral inference
//
//  Usage:
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Use the custom MistralModel loader that reads the nested JSON config
        auto model = models::mistral::MistralModel::load(weights, args.model_dir, device);
        
        // Grab the BOS ID now safely using dot operator
        int32_t bos_id = model.bos_token_id() > 0 ? model.bos_token_id() : 1;
        ...
        // Pass model by reference, removing std::move
        ...
        if (!args.chat) {
            ...
            // Mistral specific Instruct template
            std::string formatted_prompt = "[INST] " + args.prompt + " [/INST]";
            
            auto ids = tokenizer.encode(formatted_prompt, false);
            
            // Prepend the BOS token for Mistral
            ...
            while (true) {
                ...
                // Format entire history with Mistral tags
                std::string prompt;
                for (std::size_t i = 0; i < history.size(); ++i) {
                    if (history[i].role == "user") {
                        prompt += "[INST] " + history[i].content + " [/INST]";
                    } else {
